...
def autenticar(usuario, senha):
    try:
        ...
        senha_corretto = st.secrets["SENHA"]
    except:
        st.error("Erro: Credenciais não configuradas corretamente no arquivo secrets.toml.")
        return false
    return usuario == st.secrets["USUARIO"] and senha == st.secrets["SENHA"]
...
if not st.session_state["autenticado"]:
    auth_usuario = st.text_input("Usuário")
    auth_senha = st.text_input("Senha", type="password")

    if st.button("Entrar"):
        if realizar_login(auth_usuario, auth_senha):
    ...
    if "1." in opcao:
        ...
        st.subheader("Histórico Completo de Contas")
        if os.path.exists(HISTORICO_CONTAS):
            historico = pd.read_excel(HISTORICO_CONTAS, sheet_name="Histórico")
            st.dataframe(historico)
            buffer = gerar_arquivo_download(historico, "historico_completo.xlsx")
            st.download_button(
