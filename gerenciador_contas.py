...
def carregar_historico():
    if os.path.exists(HISTORICO_CONTAS):
        return pd.read_excel(HISTORICO_CONTAS, sheet_name="Histórico")
    else:
        return pd.DataFrame(columns=["Nome", "Descrição", "Valor", "Vencimento", "Situação", "Método de Pagamento", "Chave PIX", "Juros", "Data de Pagamento"])

def gerar_arquivo_download(df, nome_arquivo):
    buffer = io.BytesIO()
...
opcao = st.sidebar.selectbox("Opções", [
    "1. Adicionar Conta", 
    "2. Mostrar Sumário", 
    "3. Notificações", 
    "4. Dar Baixa", 
...
if "1." in opcao:
    ...
    with st.form("form_add_conta"):
        nome = st.text_input("Nome da Conta", value="")
        descricao = st.text_area("Descrição", value="")
        valor = st.number_input("Valor", min_value=0.0, step=0.01, value=0.0)
        ...
        pix = st.text_input("Chave PIX (se aplicável)", value="") if metodo_pagamento == "PIX" else ""
        juros = st.number_input("Juros Diário (%)", min_value=0.0, step=0.1, value=0.0)
    ...
    st.subheader("Exibir Histórico Completo")
    ...
    if historico.empty:
        ...
        st.download_button(
            ...
            data=gerar_arquivo_download(historico, "historico_completo.xlsx"),
            file_name="historico_completo.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

elif "9." in opcao:
    st.info("Obrigado por usar o sistema!")
    df = carregar_dados()
    if not df.empty:
        st.download_button(
            label="Baixar Histórico de Contas",
            data=gerar_arquivo_download(df, "historico_contas.xlsx"),
            file_name="historico_contas.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
    else:
        st.warning("Nenhum histórico para baixar.")
